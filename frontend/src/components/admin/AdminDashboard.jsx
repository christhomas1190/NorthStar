...
export default function AdminDashboard() {
    ...
    (async () => {
      ...
      try {
        ...
        const analyticsJson = analyticsRes.ok
          ...
          : null;
        const incidentsJson = incidentsRes.ok
        ...
        setAnalytics({
          ...
          byDay: Array.isArray(analyticsJson?.byDay)
            ? analyticsJson.byDay
            : [],
  ...
  // Incidents in selected date range based on "YYYY-MM-DD" from occurredAt
  ...
  // Group incidents by local calendar day for the chart
  // Group incidents by local calendar day for the chart, with zero-fill
  const computedByDay = useMemo(() => {
    const fromDate = parseLocalDay(from);
    const toDate = parseLocalDay(to);
    if (!fromDate || !toDate) return [];

    // 1) Count incidents by YYYY-MM-DD
    ...
    // 2) Build one point per day from from → to, using 0 when there are no incidents
    const points = [];
    for (
  ...
  // Severity counts from incidentsInRange
  ...
  const totalIncidentsRange =
    analytics.totalIncidents || incidentsInRange.length;
  const minor =
  ...
  // Recent Activity uses the actual occurredAt timestamp (like before)
  ...
  const tierCards = [
    {
      name: "Tier 1",
      desc: "Universal supports",
      color: "bg-emerald-50 text-emerald-700 border-emerald-200",
    },
    {
      name: "Tier 2",
      desc: "Targeted small-group supports",
      color: "bg-amber-50 text-amber-700 border-amber-200",
    },
    {
      name: "Tier 3",
      desc: "Intensive individualized supports",
  ...
  return (
    <Page
      ...
      subtitle="School overview & quick actions"
    >
      <PageTabs
        items={[
              ...
              <div className="flex items-end justify-between gap-3 flex-wrap">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="gap-1 h-9 flex items-center"
                    >
          ...
          <CardContent>
            <div className="grid gap-3 mb-6">
              <Button
                ...
                className={btnCls}
                onClick={() =>
                  (window.location.href = "/admin/define-behaviors")
                }
              >
              ...
              <Button
                ...
                className={btnCls}
                onClick={() =>
                  (window.location.href = "/admin/escalation-rules")
                }
              >
              ...
              <Button
                ...
                className={btnCls}
                onClick={() =>
                  (window.location.href = "/admin/user-role-management")
                }
              >
                  ...
                  {recentFeed.map((it) => (
                    ...
                    >
                      <span className="truncate">
                        [{it.when}] <b>{it.by}</b> recorded{" "}
                        <b>{it.category}</b> ({it.severity}) for student #
                        {it.studentId} (id {it.id})
                      </span>
                    </div>
                  ))}
                  {recentFeed.length === 0 && (
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        {tierCards.map((t) => (
          <Card
            key={t.name}
            className={`border ${t.color} shadow-sm`}
          >
            <CardHeader>
