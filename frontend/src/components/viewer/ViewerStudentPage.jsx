...
function LineChartWithAxes({ points = [], width = 600, height = 220 }) {
  ...
  const toY = (v) =>
    padTop + innerH - (v / (yTicks[yTicks.length - 1] || 1)) * innerH;
  ...
  function onMove(e) {
    ...
    setHover(
      best
        ? { x: toX(best.t), y: toY(best.c), t: best.t, c: best.c, dateStr: best.dateStr }
        : null
    );
  }
  ...
  return (
    ...
    >
      ...
      <g>
        {yTicks.map((v, i) => (
          <g key={i}>
            ...
            <text x={padLeft - 8} y={toY(v)} textAnchor="end" dominantBaseline="middle" fontSize="10" fill="#64748b">{v}</text>
          </g>
        ))}
        <line x1={padLeft} y1={padTop + innerH} x2={width - padRight} y2={padTop + innerH} stroke="#94a3b8" strokeWidth="1" />
        {xTicks.map((tk, i) => (
          <text key={i} x={toX(tk.t)} y={height - 10} textAnchor="middle" fontSize="10" fill="#64748b">{tk.label}</text>
        ))}
      </g>
      ...
      {hover && (
        <>
          ...
          <g transform={`translate(${Math.min(width - 140, Math.max(padLeft, hover.x + 8))}, ${Math.max(padTop + 8, hover.y - 30)})`}>
            ...
            <text x="8" y="14" fontSize="11" fill="#0f172a">{hover.dateStr}</text>
      ...
      <text x="12" y={padTop + innerH / 2} transform={`rotate(-90, 12, ${padTop + innerH / 2})`} textAnchor="middle" fontSize="11" fill="#334155">Count</text>
    </svg>
  );
}
...
export default function ViewerStudentPage() {
  ...
  return (
    <Page title="Student Detail" subtitle={fullName}>
      {err && (
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <div className="text-xs text-slate-500">
            <Link to="/viewer" className="underline">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            ...
            <div className="text-xs text-slate-500">
              ...
              <span className="font-semibold text-slate-700">{incidentsInRange.length}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <div className="text-xs text-slate-500 mb-2">
              Showing interventions for this student.
            </div>
            {disciplinesWithTierMarkers.length === 0 && (
              <div className="text-xs text-slate-500">No disciplines/interventions recorded.</div>
            )}
            {disciplinesWithTierMarkers.map((item, idx) => {
              if (item.kind === "tier") {
                ...
                const colorClass = !t.fromTier
                  ...
                  : "bg-slate-50 border-slate-200 text-slate-600";
                return (
                  <div key={`tier-${t.id || idx}`} className={`border rounded-lg px-2 py-1 text-xs mb-1 ${colorClass}`}>
                    <div className="flex justify-between items-center">
                      <span className="font-semibold">{arrow} {tierLabel}</span>
                      <span>{t.changedAt ? new Date(t.changedAt).toLocaleDateString() : "—"}</span>
                    </div>
                    <div className="text-slate-500 mt-0.5">by {t.changedBy || "—"}</div>
                  </div>
                );
              }
              const d = item.data;
              return (
                <div key={`dis-${d.id || idx}`} className="border rounded-lg px-2 py-1 text-xs mb-1">
                  ...
                  <div className="text-slate-500">{d.strategy || d.description || "—"}</div>
                  <div className="text-slate-400 mt-0.5">By {d.assignedBy || d.reportedBy || "—"}</div>
                </div>
              );
            })}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">All Incidents for {fullName} (in range)</CardTitle>
        </CardHeader>
            ...
            <tbody>
              {incidentsInRange.map((it) => (
                <tr key={it.id} className="border-b last:border-0">
                  ...
                  <td className="py-1 pr-2">{new Date(it.occurredAt).toLocaleString()}</td>
                  <td className="py-1 pr-2">{it.category}</td>
                  <td className="py-1 pr-2"><Badge variant="outline">{it.severity}</Badge></td>
                  <td className="py-1 pr-2 max-w-xs whitespace-pre-wrap">{it.description || "—"}</td>
